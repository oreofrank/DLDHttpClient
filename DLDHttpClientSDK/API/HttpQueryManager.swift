...
class QueryBall<T:RequestType> {
                                          ...
                                          _ success: @escaping HttpSuccess<F>)
    {
        ...
        reactive.request(params).subscribe(onSuccess: { (response) in
            let headerfileds = response.response?.allHeaderFields
            ...
            Single<Response>.just(response).filterSuccessfulStatusCodes().mapJSON().subscribe(onSuccess: { (data) in
                ...
                DispatchQueue.main.async {
                    ...
                    if let code = moyaError.response?.statusCode, code == 401 {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: loginAgainNotification), object: nil)
                    }
